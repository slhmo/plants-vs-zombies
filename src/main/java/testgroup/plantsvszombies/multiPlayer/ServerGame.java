...
public class ServerGame implements Playable {
    ...
    private void createChooserMenu() {
        ...
        play.setOnMouseClicked(event -> {
            if (chosenCards.size() == 6) {
                server.chose = true;
                if (server.allChose()) {    // initiate game
                    ...
                    sunGenerator.play();
                    zombieGenerator = new ServerZombieGenerator(this.grid, anchorPane, this);
...
//        pane.setGridLinesVisible(true);
    ...
    private class ServerZombieGenerator extends ZombieGenerator{
        public ServerZombieGenerator(Grid grid, AnchorPane anchorPane, Playable playable) {
            super(grid, anchorPane, playable);
        }
